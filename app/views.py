...
class checkout(View):
    def get(self, request):
        ...
        for p in cart_items:
            value = p.quantity * p.product.discounted_price
            famount += value
        totalamount = famount + 100  # Example: Adding a shipping fee

        # Simulate payment ID and order ID
        mock_order_id = str(uuid.uuid4())
        mock_payment_id = str(uuid.uuid4())
        mock_payment_status = "created"

        # Save mock payment details
        payment = Payment(
            user=user,
            amount=totalamount,
            ...
            paid=False,  # Payment will be marked unpaid initially
        )
        payment.save()
...
def payment_view(request):
    ...
    transaction_id = uuid4().hex  # Unique transaction ID
